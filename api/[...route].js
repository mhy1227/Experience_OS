...
var feishuAppId = env.FEISHU_APP_ID ?? "";
...
async function getFeishuToken() {
  ...
  if (Date.now() < feishuTokenExpireAt - 6e4 && feishuToken) return feishuToken;
  ...
  if (json2.code !== 0) throw new Error(`feishu token error: ${json2.msg}`);
  feishuToken = json2.tenant_access_token;
  feishuTokenExpireAt = Date.now() + json2.expire * 1e3;
  return feishuToken;
}
...
app.get("/api/feishu/tables", async (c) => {
  ...
  try {
    ...
    const json2 = await resp.json();
    if (json2.code !== 0) return c.json({ error: json2.msg }, 500);
    const tables = json2.data?.items?.map((t) => ({
      table_id: t.table_id,
      name: t.name
    })) ?? [];
...
app.get("/api/feishu/fields", async (c) => {
  ...
  try {
    ...
    const json2 = await resp.json();
    if (json2.code !== 0) return c.json({ error: json2.msg }, 500);
    const fields = json2.data?.items?.map((f) => ({
...
app.get("/api/feishu/records", async (c) => {
  ...
  try {
    ...
    do {
      ...
      const json2 = await resp.json();
      if (json2.code !== 0) return c.json({ error: json2.msg }, 500);
      const records = json2.data?.items ?? [];
      ...
      page_token = json2.data?.page_token ?? "";
...
vercelApp.all("/*", async (c) => {
  ...
  const queryString = Object.keys(query).filter((k) => query[k] !== void 0).map((k) => {
    const v = query[k];
    return Array.isArray(v) ? v.map((x) => `${k}=${encodeURIComponent(x)}`).join("&") : `${k}=${encodeURIComponent(v)}`;
  }).join("&");
  const url = queryString ? `http://localhost/api${pathname}?${queryString}` : `http://localhost/api${pathname}`;
  return app_default.fetch(new Request(url, {
    method: c.req.method,
