...
- 2026-06-22:三项 Minor 修复(`ad087cd`)。M2 markdownExport 新增 `escapeCell` 辅助:换行→空格、竖线→全角｜、反引号→全角｀,防止单元格特殊字符破坏 Markdown 表格结构,补 3 个转义测试。M3 patternDiscovery.enrichClusterWithModel 新增 `validateModelField` 校验:空/占位字面量("暂无明确根因"等)→回退统计描述,超 120 字→截断,防裸占位文本进 Insight UI,补 3 个校验测试。M1 modelClient.test 新增 `testThrowsOnMalformedJsonContent` 用例:mock content 为非 JSON,断言 completeJson 抛 SyntaxError。typecheck 0 错,14 套测试全部通过。
