...
## 10. 实机测试验收(2026-06-24,Playwright)

`typecheck` 干净、`test:evaluation` 23 套全绿;以下交互边界逐项实机走查通过、0 控制台报错:

| 用例 | 结果 |
|------|------|
| ✓有效 + 填一句 + 确认 | 记录评估,证据落库,小结刷新 |
| 取消不写入 | 点"无效"后取消,小结不变、未新增评估 |
| ?不确定 + 不填一句确认 | 记成「0 有效 / 0 无效 / 1 不确定」,空证据不报错 |
| toggle 切换 | 点"有效"再点"无效",仅"无效"高亮 |
| 重复有效 → 可信 + 沉底 | 加到 2 有效 → `✅可信`,卡片自动沉到列表底,头部「待验证」计数 −1 |
| 召回卡隐藏 / hero 显示 | 找经验召回卡 0 行快速复测(`hide-quick-retest`)、记录页 hero 卡 1 行 |
| 待验证优先排序 | `可信` 卡均沉底,`待验证` 全在上 |

**已知限制(非缺陷)**:UI 交互用 Playwright 手验,项目无组件测试框架(无 vitest),未落自动回归测试;纯逻辑(`verificationRank`/`addEvaluation`)有单测守护。若要补 UI 回归需先引入组件测试栈,本期不做。
